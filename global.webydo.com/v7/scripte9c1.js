...
function scrollToElement(elmId, ev) {
    ...
    if ($elm.length > 0 && $elm.offset()) $('html, body').animate(
    {
...
function shapelinkto(ev, link, newTab) {
    ...
    if (!$(ev.target).hasClass('dataTypeShape')
        && ev.target.getAttribute('data-type') != "Shape"
        && $(ev.target).closest('.dataTypeShape').length == 0) {
        return;
    }
    if (newTab == "true") {
